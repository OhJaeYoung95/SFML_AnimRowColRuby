...
{
	sf::Vector2f h = { INPUT_MGR.GetAxis(Axis::Horizontal), INPUT_MGR.GetAxis(Axis::Vertical) };
	...
	float absX = std::abs(h.x);
	float absY = std::abs(h.y);
	// 에니메이션
	if (animation.GetCurrentClipId() == "RowIdle")
	{
		if (h.x != 0.f && absX > absY)
		{
			animation.Play("RowMove");
		}
		if (h.y > 0.f && absX < absY)
		{
			animation.Play("DownMove");
		}
		else if (h.y < 0.f && absX < absY)
		{
